...
LinguaRecorder.prototype._getAudioStream = function() {
	...
	if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
		...
		.then(function(localMediaStream) {
			...
			// console.log('error');
			// console.log(err)
			var nerr = err.toString().split(': ');
			alert('The following error has occured: '+nerr[1]+'. Refresh your browser or contact developer if issue persists.');
...
LinguaRecorder.prototype._initStream = function() {
	...
	this.audioContext = new (window.AudioContext || window.webkitAudioContext)({sampleRate: 32000,});
...
LinguaRecorder.prototype._audioRecordingProcess = function( e ) {
	...
	this._fire( 'recording', this._audioRecord);
	...
	if ( this.timeLimit > 0 ) {
		if ( this._audioRecord.getDuration() >= this.timeLimit ) {
			this.stop();
		}
	}
};
